...
#### Allowing C to Interact with Y ####
    ...
    # Power
      Supp_N160_d0.4_rho0.4[1]
        # You should get 0.8111 for the Covariate Strategy, 0.8223 for the Kitchen Sink approach (change = 0.0112)
    
    # Type I error rate
      Supp_N160_d0_rho0.4[1]
        # You should get 0.0675 for the Covariate Strategy, 0.1182 for the Kitchen Sink approach (change = 0.0507)

#### Using the Observed Correlation in the Sample ####
    
    # We explored two variations of the Covariate Strategy that use the observed correlation between C and Y in the sample
    ...
    # When rho < .3: Power
      Supp_N160_d0.4_rho0.2[1]
        ...
        # Qualified (A) performs similarly to Baseline. Qualified (B) performs similarly to Covariate.
    ...
    # As you can see, neither Qualified (A) nor Qualified (B) offers a more favorable tradeoff than the Baseline or the Covariate
    # Using the observed correlation in the sample does not offer protection against Type I error inflation.
    # It can undercut power boost.
...
#### Adjusting Alpha Level ####
    ...
    # Find the alpha level that controls Type I error rate at .05
      dat_Supp_N160_d0_rho0.4$cov.pvalue <- pmin(dat_Supp_N160_d0_rho0.4[,13], dat_Supp_N160_d0_rho0.4[,14])
      
      adjusted.alpha <- sort(dat_Supp_N160_d0_rho0.4$cov.pvalue,
                             partial=dim(dat_Supp_N160_d0_rho0.4)[1]*0.05)[dim(dat_Supp_N160_d0_rho0.4)[1]*0.05]
      
      adj1.N160_d0.4_rho0.4 <- SimData(10000,80,80,0.4,0.4,1,1,1,1,alpha=adjusted.alpha)
      adj1.N160_d0_rho0.4 <- SimData(10000,80,80,0,0.4,1,1,1,1,alpha=adjusted.alpha)
      
    # Type I Error Rate
      adj1.N160_d0_rho0.4[1]
        # As expected, Type I error rate is exactly .05 now.
      
    # Power
      adj1.N160_d0.4_rho0.4[1]
        # You should get 0.7716 for the Covariate Strategy. Note that since we used the alpha adjustment here,
        # The baseline is not the baseline produced here; it should be the one prior to adjustment (0.7131).
        # Power boost = 0.7716 - 0.7131 = 0.0585.
      
    # Alternatively, we can directly use a smaller alpha value (e.g., alpha = .033)
      adj2.N160_d0.4_rho0.4 <- SimData(10000,80,80,0.4,0.4,1,1,1,1,alpha=0.033)
      adj2.N160_d0_rho0.4 <- SimData(10000,80,80,0,0.4,1,1,1,1,alpha=0.033)
    ...
    # Power
      adj2.N160_d0.4_rho0.4[1]
        # You should get 0.7546 for the Covariate Strategy. Power boost = 0.7546 - 0.7131 = 0.0415.
      
    # What happens when rho is smaller (e.g., rho = .1)?
      adj2.N160_d0.4_rho0.1 <- SimData(10000,80,80,0.4,0.1,1,1,1,1,alpha=0.033)
      adj2.N160_d0_rho0.1 <- SimData(10000,80,80,0,0.1,1,1,1,1,alpha=0.033)
    ...
    # Power
      ...
      # You should get 0.6663 for the Covariate Strategy. We actually have a power loss of 0.0468 now.